...
public class DataCollector extends AbstractDeviceListener {
    private double rollW =0.0;
    private double pitchW =0.0;
    private double yawW =0.0;
	...
	String xDisplay = String.valueOf(rollW) +" ";//String.format("[%s%s]", repeatCharacter('*', (int) rollW), repeatCharacter(' ', (int) (SCALE - rollW)));
	String yDisplay = String.valueOf(pitchW) +" ";//String.format("[%s%s]", repeatCharacter('*', (int) pitchW), repeatCharacter(' ', (int) (SCALE - pitchW)));
	String zDisplay = String.valueOf(yawW) +" ";//String.format("[%s%s]", repeatCharacter('*', (int) yawW), repeatCharacter(' ', (int) (SCALE - yawW)));
	...
	if (currentPose != null) {
	    ...
	    poseString = String.valueOf(currentPose.getType().ordinal());//String.format("[%s%" + (SCALE - poseTypeString.length()) + "s]", poseTypeString, " ");
	} else {
	    poseString = String.valueOf(0);//String.format("[%14s]", " ");
	}
    ...
    public double[] outD()
    {
    	double[] val = {rollW,pitchW,yawW,currentPose.getType().ordinal()};
    	return val;
    }
