...
public final class Hub {
	...
	public Hub(String applicationIdentifier, String pathdll) {
		this.applicationIdentifier = applicationIdentifier;
		path2dll=pathdll;
		loadJniResources();
		initialize(applicationIdentifier);
	...
	private boolean copyAndLoadOSXFromTemp() {
		try {
			...
			try (InputStream macMyoInputStream = this.getClass().getResourceAsStream(path2dll+"/osx/libmyo.jnilib")) {
				Files.copy(macMyoInputStream, macMyoTempFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}
			macMyoTempFile.deleteOnExit();

			try (InputStream macMyoLibInputStream = this.getClass().getResourceAsStream(path2dll+"/osx/myo.zip")) {
	...
	private boolean loadX64ResourcesFromSysPath() throws UnsatisfiedLinkError {
		try {
			System.load(path2dll+"/x64/myo64.dll");
			System.load(path2dll+"/x64/JNIJavaMyoLib.dll");
			return true;
		} catch (UnsatisfiedLinkError e) {
	...
	private boolean loadWin32ResourcesFromSysPath() throws UnsatisfiedLinkError {
		try {
			System.load(path2dll+"/Win32/myo32.dll");
			System.load(path2dll+"/Win32/JNIJavaMyoLib.dll");
			return true;
		} catch (UnsatisfiedLinkError e) {
	...
	private boolean copyAndLoadX64FromTemp() {
		try {
			...
			try (InputStream myo64DllInputStream = this.getClass().getResourceAsStream(path2dll+"/x64/myo64.dll")) {
			...
			try (InputStream jniJavaMyoLibDllInputStream = this.getClass().getResourceAsStream(path2dll+"/x64/JNIJavaMyoLib.dll")) {
				//Files.copy(jniJavaMyoLibDllInputStream, jniJavaMyoLibDllTempFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}

			System.load(path2dll+"/x64/myo64.dll");
			System.load(path2dll+"/x64/JNIJavaMyoLib64.dll");
			return true;
		} catch (UnsatisfiedLinkError e) {
	...
	private boolean copyAndLoadWin32FromTemp() {
		try {
			...
			try (InputStream myo32DllInputStream = this.getClass().getResourceAsStream(path2dll+"/Win32/myo32.dll")) {
			...
			try (InputStream jniJavaMyoLibDllInputStream = this.getClass().getResourceAsStream(path2dll+"/Win32/JNIJavaMyoLib.dll")) {
				//Files.copy(jniJavaMyoLibDllInputStream, jniJavaMyoLibDllTempFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}

			System.load(path2dll+"/Win32/myo32.dll");
			System.load(path2dll+"/Win32/JNIJavaMyoLib32.dll");
			return true;
		} catch (UnsatisfiedLinkError e) {
